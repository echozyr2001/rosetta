use rosetta::nom_parser::{parse, parse_with_config, NomParser};
...
"#;
    ...
    println!("Parsed {} blocks from complex document:", complex_doc.blocks.len());
    ...
    match parse(malformed_markdown) {
        Ok(doc) => {
            println!("Successfully parsed malformed input with {} blocks", doc.blocks.len());
    ...
    let unicode_markdown = "# 🌍 Unicode Title\n\nText with émojis 🎉 and àccénts.\n\n- 日本語\n- العربية\n- Русский";
    let unicode_doc = parse(unicode_markdown)?;
    
    println!("Unicode document parsed {} blocks:", unicode_doc.blocks.len());
    for (i, block) in unicode_doc.blocks.iter().enumerate() {
        match block {
            ...
            Block::Paragraph { content, .. } => {
                if let Some(Inline::Text(text)) = content.first() {
                    println!("  Paragraph {}: {}", i, text.chars().take(30).collect::<String>());
                }
            }
            Block::List { items, .. } => {
    ...
    for (i, inline) in content.iter().enumerate() {
        match inline {
            ...
            Inline::Emphasis { strong, content } => {
                println!("{}Inline {}: Emphasis(strong: {}, {} items)", spaces, i, strong, content.len());
            }
            Inline::Code(code) => println!("{}Inline {}: Code(\"{}\")", spaces, i, code),
            Inline::Link { text, destination, title } => {
                println!("{}Inline {}: Link(dest: \"{}\", title: {:?}, {} text items)", 
                         spaces, i, destination, title, text.len());
            }
            Inline::Image { alt, destination, title } => {
                println!("{}Inline {}: Image(alt: \"{}\", dest: \"{}\", title: {:?})", 
                         spaces, i, alt, destination, title);
            }
            Inline::HtmlInline(html) => println!("{}Inline {}: HtmlInline(\"{}\")", spaces, i, html),
