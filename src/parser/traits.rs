...
/// Trait implemented by concrete parsing rules.
///
/// A parsing rule consumes tokens through `ParserState` and produces a document
/// AST. Concrete implementations are free to decide how much of the token slice
/// they consume.
pub trait ParseRule<'a, Tok, Root>
where
    Tok: crate::lexer::LexToken + 'a,
    Root: AstNode,
{
    fn parse(&self, input: TokenSlice<'a, Tok>) -> IResult<TokenSlice<'a, Tok>, Root>;
}
