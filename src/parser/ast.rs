...
/// Identifier used by the new tree-based AST nodes.
pub type NodeId = usize;

/// Payload stored inside each AST node.
#[derive(Debug, Clone)]
pub enum NodeKind {
    Document(Document),
    Block(Block),
...
/// Tree node that stores hierarchical relationships and metadata.
#[derive(Debug, Clone)]
pub struct Node {
    pub id: NodeId,
...
impl Node {
    pub fn new(id: NodeId, kind: NodeKind, position: Option<Position>) -> Self {
        Self {
            id,
            parent: None,
            children: Vec::new(),
